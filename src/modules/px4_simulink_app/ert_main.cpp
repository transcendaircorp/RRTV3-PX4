...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(RRTV3_PX4_OA_M) == (NULL));
  while (runModel) {
    ...
    if (rtmStepTask(RRTV3_PX4_OA_M, 1)
        ) {
    ...
    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(RRTV3_PX4_OA_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(RRTV3_PX4_OA_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(RRTV3_PX4_OA_M, 0);

  // Initialize model
  RRTV3_PX4_OA_initialize();

  // Call RTOS Initialization function
  nuttxRTOSInit(0.02, 1);
